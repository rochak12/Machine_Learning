...
data <-read.csv(file = "C:/Users/Rochak/Desktop/Machine Learning Exercise/letters.csv", head=TRUE, sep= ",")
print(data)
...
letterTest <- data[testRows,]

# The training set contains all the other rows
letterTrain <- data[-testRows,]
...
# Please note that "letter~." contains a tilde character, rather than a minus
model <- svm(letter~., data = letterTrain, kernel = "radial", gamma = 1, cost = 50, type ="C")
